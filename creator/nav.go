...
func (t *toc) writeNavDoc(tempDir string) {
	...
	navFilePath := filepath.Join(tempDir, contentFolderName, textFolderName, tocNavFilename)
	n.write(navFilePath)
}
...
func (t *toc) writeNcxDoc(tempDir string) {
	...
	if err := os.WriteFile(ncxFilePath, []byte(ncxFileContent), filePermissions); err != nil {
